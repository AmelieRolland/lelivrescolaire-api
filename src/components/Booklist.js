                    ...
                    {filteredBooks.map((book) => (
                        <div key={book.id} className={"group flex flex-row transition duration-700 ease-in-out max-w-full h-auto border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700" + (book.valid === true ? ' bg-white' : ' hidden')}>
                            <div className="flex-shrink-0 w-1/3">
                                <img src={book.urlLite} alt={book.displayTitle} className="w-full h-full object-cover rounded-l-lg transform transition-transform duration-700 ease-in-out group-hover:rotate-6" />
                            </div>
                            <div className='flex flex-col justify-between p-4 w-2/3'>
                                ...
                                <div className='mt-2'>
                                    <a href={`/book/${book.id}`} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-[#ffa800] rounded-lg hover:bg-yellow-400 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                        Ouvrir
